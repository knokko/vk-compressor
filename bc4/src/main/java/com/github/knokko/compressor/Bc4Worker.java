...
/**
 * A worker of the <i>Bc4Compressor</i>. This class contains some potentially mutable state that is needed for the
 * compression. Therefor, you need multiple <i>Bc4Worker</i>s if you want to record in parallel.
 */
public class Bc4Worker {
	 ...
	 */
	public Bc4Worker(Bc4Compressor compressor, int maxDestinationImagePixels, MemoryCombiner combiner) {
	 ...
	 * @param source The source buffer containing the 8-bit grayscale image data
	 * @param destination The destination image with BC4 format, must be in <i>VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL</i>
	 */
	...
	public void compress(
		...
		try (MemoryStack stack = stackPush()) {
			...
			vkCmdPushConstants(
					...
					VK_SHADER_STAGE_COMPUTE_BIT, 0, recorder.stack.ints(bigEndian, width, height)
			);
		}
